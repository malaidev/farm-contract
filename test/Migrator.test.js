...
contract('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.farm = await BirdFarm.new(this.egg.address, '500000', '1000', '0', '100000', { from: alice });
        this.migrator = await Migrator.new(this.farm.address, this.factory1.address, this.factory2.address, '0');
        await this.egg.transferOwnership(this.farm.address, { from: alice });
        await this.farm.add('100', this.lp1.address, true, { from: alice });
    });
    ...
    it('should do the migration successfully', async () => {
        ...
        await this.lp1.sync();
        await this.lp1.approve(this.farm.address, '100000000000', { from: minter });
        await this.farm.deposit('0', '2000000', { from: minter });
        assert.equal((await this.lp1.balanceOf(this.farm.address)).valueOf(), '2000000');
        await expectRevert(this.farm.migrate(0), 'migrate: no migrator');
        await this.farm.setMigrator(this.migrator.address, { from: alice });
        await expectRevert(this.farm.migrate(0), 'migrate: bad');
        await this.factory2.setMigrator(this.migrator.address, { from: alice });
        await this.farm.migrate(0);
        assert.equal((await this.lp1.balanceOf(this.farm.address)).valueOf(), '0');
        assert.equal((await this.lp2.balanceOf(this.farm.address)).valueOf(), '2000000');
        await this.farm.withdraw('0', '2000000', { from: minter });
