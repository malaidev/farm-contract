...
// BirdFarm is the master of Egg. He can make Egg and he is a fair guy.
//
...
contract BirdFarm is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        require((egg.totalSupply().add(eggReward)) < cap, "cap is reached");
    ...
    function increaseSupply(uint256 _supply) external onlyOwner {
        require((egg.totalSupply().add(_supply)) < cap, "cap is reached");
        
        egg.mint(address(this), _supply);
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            pool.accEggPerShare = pool.accEggPerShare.add(_supply.mul(1e12).div(lpSupply));
            pool.lastRewardBlock = block.number;
        }
